...
export default function EditPostForm({ post }: { post: TPost }) {
  ...
  useEffect(() => {
    const featchAllCategories = async () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !content) {
        toast.error("Title and content are requeried");
      return;
    }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2">
        ...
        {publicId && (
          <button
            onClick={removeImage}
            className="py-2 px-4 bg-red-600 w-fit rounded-md text-white font-bold mb-3"
          >
            {" "}
            Remove Image
          </button>
        ...
        <select
          ...
          className="p-3 border rounded-md appearance-none"
          value={selectedCategory}
        >
          <option value="">Select A Cotegory</option>
          {categories &&
            categories.map((categor) => (
              <option key={categor.id} value={categor.catName}>
                {categor.catName}
              </option>
            ))}
        </select>
